...
function navigateTo(pageId) {
  ...
  if (target) {
    ...
    const fallback = document.querySelector('.page[data-page="prototype-7"]');
  ...
  // Auto-init V5 Tamagochi prototype
  if (pageId === 'prototype-tamagochi') {
...
window.addEventListener('hashchange', () => {
  const pageId = window.location.hash.replace('#', '') || 'prototype-7';
  navigateTo(pageId);
});

// Initial route on page load — Tamagochi is the new home page
const initialPage = window.location.hash.replace('#', '') || 'prototype-7';
...
// =============================================
// ======================================================
// Shared world-map bitmap (Natural Earth 110m, 360×180)
// Lazy-decoded once, reused by prototype-7 globe + Tamagochi pixel map
// ======================================================
var _worldBitmapCache = null;
function getWorldBitmap() {
  if (_worldBitmapCache) return _worldBitmapCache;
  // Bitmap copy lives inside initP7Globe. We share it via a small helper
  // that decodes on first call. The base64 string itself is declared inside
  // initP7Globe and assigned to window.__mapB64__ when initProto7 runs so the
  // same bytes aren't held in two places; if the user lands on Tamagochi first,
  // we fall back to decoding from __mapB64__ which initProto7 loads lazily.
  if (typeof window.__mapB64__ !== 'string') {
    // Proto-7 hasn't run yet — trigger a minimal decode path by initializing
    // the cache lazily from whatever the initP7Globe stored.
    return null;
  }
  var raw = atob(window.__mapB64__);
  ...
  _worldBitmapCache = {
    ...
    W: 360,
    H: 180,
    isLand: function(lat, lng) {
      var col = Math.round((lng + 180) / 360 * 359);
      var row = Math.round((90 - lat) / 180 * 179);
      if (col < 0 || col >= 360 || row < 0 || row >= 180) return false;
      var idx = row * 360 + col;
      return (bytes[idx >> 3] >> (7 - (idx & 7))) & 1;
    }
  };
  return _worldBitmapCache;
}
...
function initProto7() {
  ...
  function initP7Globe() {
    ...
    // Share the bitmap string with the Tamagochi pixel-art map (and any
    // other prototype that wants real geography) without duplicating it.
    window.__mapB64__ = mapB64;
  ...
  // ---- FOOTPRINT BUTTON (collab mode overlay) ----
  var fpBtn = document.getElementById('p7FootprintBtn');
  var fpOverlay = document.getElementById('p7FootprintOverlay');
  var fpBack = document.getElementById('p7FootprintOverlayBack');
  var fpMapInstance = null;
  if (fpBtn && fpOverlay) {
    fpBtn.addEventListener('click', function() {
      if (typeof renderTripHistory === 'function') renderTripHistory();
      var listSrc = document.getElementById('p7HistoryList');
      var listDest = document.getElementById('p7FootprintOverlayList');
      if (listSrc && listDest) listDest.innerHTML = listSrc.innerHTML;
      fpOverlay.classList.add('open');
      // Render Leaflet map in overlay
      var mapEl = document.getElementById('p7FootprintOverlayMap');
      if (mapEl && typeof L !== 'undefined') {
        if (fpMapInstance) { fpMapInstance.remove(); fpMapInstance = null; }
        mapEl.innerHTML = '';
        setTimeout(function() {
          var h = p7RecordedHistory;
          if (!h) {
            h = {
              ...
              points: 185, distance: 2.4, elapsed: 3600
            };
          }
          var center = h.trail[Math.floor(h.trail.length / 2)] || [35.6716, 139.6966];
          fpMapInstance = L.map(mapEl, { center: center, zoom: 15, zoomControl: false, attributionControl: false, dragging: true, scrollWheelZoom: false });
          L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', { maxZoom: 19 }).addTo(fpMapInstance);
          if (h.trail.length) {
            L.polyline(h.trail, { color: '#7C3AED', weight: 4, opacity: 0.85, lineCap: 'round', lineJoin: 'round' }).addTo(fpMapInstance);
          }
          h.visited.forEach(function(v, i) {
            L.marker([v.lat, v.lng], {
              icon: L.divIcon({
          ...
          if (h.trail.length) {
            fpMapInstance.fitBounds(L.latLngBounds(h.trail).pad(0.1));
          }
        }, 150);
      }
    });
  ...
  if (fpBack && fpOverlay) {
    fpBack.addEventListener('click', function() {
  ...
  var spotsImportBtn = document.getElementById('p7SpotsImportBtn');
  if (spotsImportBtn) {
    spotsImportBtn.addEventListener('click', function() {
  ...
  function renderTripSpots() {
    ...
    spots.forEach(function(s, idx) {
      ...
      var av = spotAvatars[idx % spotAvatars.length];
      html += '<div class="p7-spot-avatar" style="background:' + av.bg + '">' + av.initial + '</div>';
...
// ======================================================
// V5 · Tamagochi — pixel-art travel companion
// ======================================================
var pTamaInitialized = false;
function initProtoTamagochi() {
  if (pTamaInitialized) return;
  pTamaInitialized = true;

  var continueBtn = document.getElementById('pTamaContinueBtn');
  var travelerContinueBtn = document.getElementById('pTamaTravelerContinueBtn');
  if (!continueBtn) return;

  var bubble = document.querySelector('#prototype-tamagochi .p-tama-bubble');
  var pet = document.querySelector('#prototype-tamagochi .p-tama-pet');
  var phrases = ['Hello!', "Let's go!", 'Woof!', 'Ready?', 'Arf!'];
  var phraseIdx = 0;

  // Cycle speech bubble phrases every ~4 seconds — another subtle "game" beat.
  setInterval(function() {
    if (!bubble) return;
    phraseIdx = (phraseIdx + 1) % phrases.length;
    bubble.style.opacity = '0';
    setTimeout(function() {
      bubble.textContent = phrases[phraseIdx];
      bubble.style.opacity = '1';
    }, 140);
  }, 4000);

  // View switching helper
  function pTamaShowView(viewName) {
    var views = document.querySelectorAll('#prototype-tamagochi .p-tama-view');
    views.forEach(function(v) {
      v.classList.toggle('p-tama-view-active', v.getAttribute('data-p-tama-view') === viewName);
    });
    // Reset scroll of the traveler scene when entering
    if (viewName === 'traveler') {
      var scene = document.querySelector('#prototype-tamagochi .p-tama-traveler-scene');
  ...
  continueBtn.addEventListener('click', function() {
    if (pet) {
      // Excited double-hop reaction before the transition
      pet.style.animation = 'none';
      void pet.offsetWidth;
      pet.style.animation = 'p-tama-pet-idle 0.5s steps(3, end) 2';
      setTimeout(function() { pet.style.animation = ''; }, 1100);
    }
    if (bubble) bubble.textContent = "Let's go!";
    // Wait briefly so the hop reads, then transition
    setTimeout(function() { pTamaShowView('traveler'); }, 420);
  });

  // Screen 2 → Screen 3 (the Zenvoya app home)
  if (travelerContinueBtn) {
    travelerContinueBtn.addEventListener('click', function() {
      pTamaShowView('home');
    });
  }

  // Back button on traveler screen → back to meet screen
  var backBtn = document.getElementById('pTamaBackBtn');
  if (backBtn) {
    backBtn.addEventListener('click', function() {
      pTamaShowView('meet');
      if (bubble) bubble.textContent = 'Hello!';
      phraseIdx = 0;
    });
  }

  // Room / Map toggle on the home screen
  var roomScene = document.querySelector('#prototype-tamagochi .p-tama-room');
  document.querySelectorAll('#prototype-tamagochi .p-tama-room-tab').forEach(function(tab) {
    tab.addEventListener('click', function() {
      document.querySelectorAll('#prototype-tamagochi .p-tama-room-tab').forEach(function(t) {
        t.classList.remove('p-tama-room-tab-active');
      });
      this.classList.add('p-tama-room-tab-active');
      var mode = this.getAttribute('data-room-tab');
      if (roomScene) roomScene.setAttribute('data-room-mode', mode);
      if (mode === 'map') drawPTamaWorldMap();
    });
  });

  // Bottom-nav click → switch between home and map-spots views
  document.querySelectorAll('#prototype-tamagochi [data-tama-nav]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      var target = this.getAttribute('data-tama-nav');
      pTamaShowView(target);
    });
  });

  // Togo Shrine card → open modal; backdrop/close → close
  var togoCard = document.getElementById('pTamaTogoCard');
  var togoModal = document.getElementById('pTamaTogoModal');
  var togoClose = document.getElementById('pTamaTogoClose');
  var togoBackdrop = document.getElementById('pTamaTogoBackdrop');
  if (togoCard && togoModal) {
    togoCard.addEventListener('click', function() {
      togoModal.classList.add('p-tama-togo-modal-open');
      togoModal.setAttribute('aria-hidden', 'false');
    });
    function closeTogoModal() {
      togoModal.classList.remove('p-tama-togo-modal-open');
      togoModal.setAttribute('aria-hidden', 'true');
    }
    if (togoClose) togoClose.addEventListener('click', closeTogoModal);
    if (togoBackdrop) togoBackdrop.addEventListener('click', closeTogoModal);
  }

  // Pixel world map — sample from Natural Earth bitmap for real geography
  // Visited countries, lat/lng pairs
  var worldPins = [
    { name: 'Japan',  lat: 35.68, lng: 139.65 },   // Tokyo
    { name: 'Spain',  lat: 40.42, lng:  -3.70 },   // Madrid
    { name: 'France', lat: 48.85, lng:   2.35 },   // Paris
    { name: 'UK',     lat: 51.51, lng:  -0.13 },   // London
    { name: 'USA',    lat: 40.71, lng: -74.00 }    // NYC
  ];

  var worldMapDrawn = false;
  var worldBitmap = null;

  // Deterministic water-noise dots for a retro ocean texture
  var waterNoiseCache = null;
  function getWaterNoise(w, h, count) {
    if (waterNoiseCache) return waterNoiseCache;
    waterNoiseCache = [];
    var seed = 1337;
    function rand() { seed = (seed * 16807) % 2147483647; return seed / 2147483647; }
    for (var i = 0; i < count; i++) {
      waterNoiseCache.push([Math.floor(rand() * w), Math.floor(rand() * h)]);
    }
    return waterNoiseCache;
  }

  function drawPTamaWorldMap() {
    var canvas = document.getElementById('pTamaWorldMapCanvas');
    if (!canvas) return;
    if (worldMapDrawn) return; // pins animate via their own loop below
    worldBitmap = getWorldBitmap();
    if (!worldBitmap) return;  // bitmap asset not loaded yet
    worldMapDrawn = true;

    var ctx = canvas.getContext('2d');
    var W = canvas.width, H = canvas.height;

    // Convert pin lat/lng to canvas pixel coordinates
    function pinXY(pin) {
      return [
        Math.round((pin.lng + 180) / 360 * W),
        Math.round((90 - pin.lat) / 180 * H)
      ];
    }
    var pinPx = worldPins.map(function(p) { return pinXY(p); });

    // Determine for each canvas pixel if it's land, by sampling the
    // Natural Earth 360×180 bitmap via equirectangular projection
    function isLandPx(x, y) {
      var lng = (x / W) * 360 - 180;
      var lat = 90 - (y / H) * 180;
      return worldBitmap.isLand(lat, lng);
    }

    // Retro palette — soft highlight / body / drop shadow
    var landHi = '#8FD26B';
    var landBody = '#4E8C3A';
    var landShadow = '#2A5D1F';

    function renderMap() {
      // Sky/ocean base gradient is drawn by CSS; we just paint land+pins
      ctx.clearRect(0, 0, W, H);

      // Sparse water noise
      ctx.fillStyle = 'rgba(255, 255, 255, 0.28)';
      getWaterNoise(W, H, 80).forEach(function(p) {
        ctx.fillRect(p[0], p[1], 1, 1);
      });

      // Fill land with 3-tone shading based on neighbors
      for (var y = 0; y < H; y++) {
        for (var x = 0; x < W; x++) {
          if (!isLandPx(x, y)) continue;
          var above = y > 0 && isLandPx(x, y - 1);
          var below = y < H - 1 && isLandPx(x, y + 1);
          ctx.fillStyle = !above ? landHi : (!below ? landShadow : landBody);
          ctx.fillRect(x, y, 1, 1);
        }
      }

      // Pins — red core with yellow pulse ring
      var pulseFrame = Math.floor(Date.now() / 450) % 3; // 0,1,2
      pinPx.forEach(function(p) {
        if (pulseFrame !== 2) {
          // Outer ring, 2px offset
          ctx.fillStyle = 'rgba(255, 217, 0, 0.85)';
        ...
        ctx.fillRect(p[0], p[1], 1, 1);
        // White highlight pixel for depth
        ctx.fillStyle = '#FFFFFF';
        ctx.fillRect(p[0], p[1], 1, 1);
        ctx.fillStyle = '#E02020';
        ctx.fillRect(p[0], p[1] + 1, 1, 1);
      });
    }

    renderMap();
    setInterval(renderMap, 450);
  }

  // Home-screen welcome bubble — stream friendly messages every 5s
  var welcomeBubble = document.querySelector('#prototype-tamagochi .p-tama-welcome-bubble');
  if (welcomeBubble) {
    var welcomeMessages = [
      'Welcome back, George!',
    ...
    // Lazily add a blinking caret inside the bubble
    function ensureCaret() {
      if (!caretEl) {
        caretEl = document.createElement('span');
        caretEl.className = 'p-tama-caret';
        welcomeBubble.appendChild(caretEl);
      }
      return caretEl;
    }

    // Set text as a text node + caret at the end (so the blinking caret doesn't
    // get clobbered by textContent writes during typing)
    function setBubbleText(text) {
      // Clear all child nodes except the caret
      while (welcomeBubble.firstChild) welcomeBubble.removeChild(welcomeBubble.firstChild);
      welcomeBubble.appendChild(document.createTextNode(text));
      welcomeBubble.appendChild(ensureCaret());
    }

    function streamNext() {
      if (typingTimer) { clearInterval(typingTimer); typingTimer = null; }
      welcomeIdx = (welcomeIdx + 1) % welcomeMessages.length;
      var target = welcomeMessages[welcomeIdx];
      var i = 0;
      setBubbleText('');
      // ~28ms per char feels like a chatty pet typing, not too slow
      typingTimer = setInterval(function() {
        i++;
        setBubbleText(target.slice(0, i));
        if (i >= target.length) {
    ...
    // Set the initial text with a caret
    setBubbleText(welcomeMessages[0]);
    setInterval(streamNext, 5000);
  }
}
